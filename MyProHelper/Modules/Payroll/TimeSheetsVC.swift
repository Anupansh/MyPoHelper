...
//  MyProHelper
//
//  Created by Anupansh on 26/07/21.
...
//        }
    ...
    func fetchList(key: String) {
        ...
        for singleData in currentTimeSheetModel {
            if (    DateManager.standardDateToStringWithoutHours(date: singleData.dateWorked).contains(key) ||
    ...
    func presentActionSheet(index: Int,indexpath: IndexPath) {
        if !toShowRemoved {
            if AppLocals.workerRole.role.canApprovePayroll! {
                ActionSheetManager.shared.showActionSheet(typeOfAction: [.approveAction,.viewAction,.editAction,.deleteAction], presentingView: dataTable.collectionView.cellForItem(at: indexpath)!.contentView, vc: self, actionPerformed: { [weak self] actionPerformed in
                    ...
                    else if actionPerformed == .approveAction {
