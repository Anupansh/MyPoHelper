...
class CreateWorkOrderViewModel: BaseAttachmentViewModel {
...
//                self.customers = customers
    ...
    func canEditWorker() -> Bool {
        return workOrder.workOrderId == nil || workOrder.workOrderId != nil && self.isCanEditTimeAlreadyEntered
    }
    ...
    func validateCustomer() -> ValidationResult {
        ...
        /*
        if workOrder.customerId == nil && workOrder.customerName == nil{
            return .Invalid(message: Constants.Message.NAME_SELECT_ERROR)
        }

        if let customerName = workOrder.customerName , workOrder.customerId == nil{
        ...
        if let customerName = workOrder.customerName{
            if customerName == Constants.DefaultValue.SELECT_LIST{
    ...
    private func updateWorkOrder(completion: @escaping (_ error: String?) -> ()) {
        ...
        workOrderService.updateWorkOrder(workOrder: workOrder) { [weak self] (result) in
            guard let self = self else { return }
            switch result {
            case .success:
