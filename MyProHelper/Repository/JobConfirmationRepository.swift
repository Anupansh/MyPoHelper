...
class JobConfirmationRepository: BaseRepository {
    ...
    func updateJob(job: Job, success: @escaping () -> (), failure: @escaping (_ error: Error) -> ()) {
        ...
        AppDatabase.shared.executeSQL(sql: sql,
                                     arguments: arguments,
                                     typeOfAction: .update, updatedId: UInt64(job.jobID ?? 0),
                                     success: { _ in
                                        success()
                                     },
                                     fail: failure)
    ...
    func fetchJob(showRemoved: Bool, with key: String? = nil, sortBy: JobConfirmationField? = nil, sortType: SortType? = nil ,offset: Int,success: @escaping(_ jobs: [Job]) -> (), failure: @escaping(_ error: Error) -> ()) {
        ...
        WHERE (main.\(tableName).\(COLUMNS.JOB_STATUS) LIKE 'Waiting')
        \(condition)
        """
...
//        LEFT JOIN main.\(TABLES.CUSTOMERS) ON main.\(tableName).\(COLUMNS.CUSTOMER_ID) == main.\(TABLES.CUSTOMERS).\(COLUMNS.CUSTOMER_ID)
//        WHERE (main.\(tableName).\(COLUMNS.JOB_STATUS) LIKE 'Waiting')
//        OR (main.\(tableName).\(COLUMNS.JOB_STATUS) LIKE 'Rejected')
//        \(condition)
//        """
    ...
    func fetchJob(showRemoved: Bool, with key: String? = nil, sortBy: JobsToRescheduleViewField? = nil, sortType: SortType? = nil ,offset: Int,success: @escaping(_ jobs: [Job]) -> (), failure: @escaping(_ error: Error) -> ()) {
        guard let queue = AppDatabase.shared.attachDababaseQueue else { return }
        let sortable = makeSortableItems2(sortBy: sortBy, sortType: sortType)
        let searchable = makeSearchableItems(key: key)
        
        let removedCondition = """
                AND (main.\(tableName).\(COLUMNS.REMOVED) = 0
                OR main.\(tableName).\(COLUMNS.REMOVED) is NULL)
                """
        
        let showRemovedCondition = """
                                AND (main.\(tableName).\(COLUMNS.REMOVED) = 1)
                                """
        var condition = ""
...
//        condition = (searchable.isEmpty) ? "" : "AND \(searchable)"
        
        var sql = """
        SELECT * FROM main.\(tableName)
        LEFT JOIN main.\(TABLES.WORKERS) ON main.\(tableName).\(COLUMNS.WORKER_SCHEDULED) == main.\(TABLES.WORKERS).\(COLUMNS.WORKER_ID)
        LEFT JOIN main.\(TABLES.CUSTOMERS) ON main.\(tableName).\(COLUMNS.CUSTOMER_ID) == main.\(TABLES.CUSTOMERS).\(COLUMNS.CUSTOMER_ID)
        WHERE (main.\(tableName).\(COLUMNS.JOB_STATUS) LIKE 'Rejected')
        \(condition)
        """
    ...
    private func makeSortableItems(sortBy: JobConfirmationField?, sortType: SortType?) -> String {
        guard let sortBy = sortBy, let sortType = sortType else {
            return makeSortableCondition(key: COLUMNS.LAST_NAME, sortType: .ASCENDING)
        }
        switch sortBy {
    ...
    private func makeSortableItems2(sortBy: JobsToRescheduleViewField?, sortType: SortType?) -> String {
        guard let sortBy = sortBy, let sortType = sortType else {
